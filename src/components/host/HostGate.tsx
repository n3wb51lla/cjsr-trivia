...
export function HostGate({ title, description, children }: { title: string; description?: string; children: ReactNode }) {
  ...
  return (
    <section className="page-card p-6">
      ...
      {description && <p className="mt-3 max-w-2xl text-brand-paper">{description}</p>}
      <form className="mt-6 max-w-sm" onSubmit={unlock}>
        ...
        <input
          id="host-passphrase"
          type="password"
          className="mt-2 min-h-11 w-full border-2 border-brand-red bg-brand-black px-3 py-2 text-brand-ink"
          value={passphrase}
          onChange={event => setPassphrase(event.target.value)}
        />
        <button type="submit" className="mt-4 min-h-11 border-2 border-brand-red bg-brand-red px-5 py-2 font-black text-white">
          Unlock
        </button>
      </form>
      {error && <p className="mt-4 font-bold text-brand-red-light" role="alert">{error}</p>}
    </section>
  );
}
