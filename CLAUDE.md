...
This repo contains Trivia Knight, a live multiplayer trivia product evolved from an earlier single-event proof of concept. The working application uses the Trivia Knight name and Claude Design's navy/gold/off-white product system across player, host, projector, and marketing surfaces. A previous Firebase project is still connected through the local environment, so do not deploy or mutate live game state without explicit approval.
...
- All product surfaces use text-only Trivia Knight branding until the redesigned logo is supplied. Do not introduce a placeholder logo as production source material.
...
- `26395b5 Fix` (fixes a real `database.rules.json` bug: `.numChildren()` isn't a valid Firebase RTDB rules-language method — it's client-SDK-only — so the multi-select `answers`, free-text `acceptedAnswers`, and `choiceIndexes` cardinality checks failed rules-syntax validation on deploy. Replaced with the standard RTDB workaround: check index `0` exists for the lower bound, check the one-past-max index doesn't exist for the upper bound. Despite the generic message, this was a deploy-blocking fix, not a no-op — see "Firebase Notes" below.)
- `589b697 v2 Cont 2` (the a11y-scan pass: required `altText`/optional `captionsUrl` on `QuestionMedia`, radiogroup semantics for multiple-choice answers, nested-`<label>` fix in the host editor — see "Accessibility" under Known Follow-Ups for full detail)
...
There is also a `template` branch (`f01b52b` as of last sync), branched from `main`, with customer-specific files removed (see "Product Direction"). It's periodically merged forward from `main` to pick up generic improvements — check its own git log for how current it is relative to `main`. As of this commit, `template` was last confirmed caught up to `main` through E1 — it has **not** been re-synced since, so `template` is behind `main` by everything from `2e569a9` onward (free-text team naming, E2a/E2b, E5, E6, Stage 1, and the a11y-scan pass).

A previous Firebase deployment was confirmed current as of `26395b5` on 2026-07-18. That deployment predates the Trivia Knight rebrand. Treat it only as historical infrastructure state, not a deployment target for this working tree.
...
The local application now includes the marketing page, product walkthrough, white-labelled live game surfaces, and a host-only brand builder. None of this work has been deployed.

### Current local work (not deployed)

This working tree is the current Trivia Knight product direction and supersedes the retired single-event skin:
...
- **Logo constraint:** every working surface uses a text-only “Trivia Knight” wordmark. The design package's temporary knight/chess assets are not imported or bundled. Wait for the owner's redesigned logo.
...
- **Theme control:** the retired cross glyph is gone. The accessible control now uses the text labels “Light” and “Dark”; theme persistence is under `trivia-knight.theme`.
- **Sample data:** the customer-specific sudden-death station-frequency question was replaced with a generic chessboard question. The remaining Canadian-music questions are sample content, not interface branding.
...
- The current product deliberately uses a **text-only** `Trivia Knight` brand treatment. No legacy customer logo remains in the repository.
...
- The deployed POC at `the legacy Firebase Hosting site` was visually reviewed read-only at `/`, `/host`, and `/screen` after the marketing page was built. No passphrase was entered and no Firebase state was changed.
- Treat the deployed/current original POC app and this file's capability record as the **functional** source of truth. Its player join sequence, live-connection state, host gate, and projector states should be preserved when translating product surfaces into the TriviaKnight brand.
...
- Before any production deployment, decide explicitly whether `/marketing` stays a separate route, becomes the public `/` route with the player experience moved elsewhere, or ships as a separate marketing site/domain. Do not overwrite the live original POC homepage by assumption.
...
- RTDB rules target: `the legacy RTDB instance`
...
- Multi-select `answers`, free-text `acceptedAnswers`, and `choiceIndexes` cardinality bounds, and the E6/a11y-scan `media.altText`/`media.captionsUrl` shape checks — deployed as of `26395b5`

**Rules-language gotcha (real bug, cost a deploy attempt):** `.numChildren()` is **not** part of the Firebase RTDB rules language — it's a `DataSnapshot` client-SDK method only; the rules API's `RuleDataSnapshot` only exposes `exists()`, `hasChild()`, `hasChildren()`, `val()`, `isNumber()`/`isString()`/`isBoolean()`, `child()`, `parent()`, `getPriority()`. The E2a/E2b rules additions used `.numChildren()` to bound array cardinality (`answers`, `acceptedAnswers`, `choiceIndexes`) and were never actually deployable — this went unnoticed because those rules weren't deployed until the `26395b5` fix. Firebase RTDB arrays serialize to objects keyed by stringified index, so the correct pattern for bounding cardinality without `numChildren()` is: check `.child('0').exists()` for a lower bound of 1, and `!.child('N').exists()` (where `N` is one past the max) for an upper bound. If adding any new array-shaped field to `database.rules.json`, use this pattern, not `numChildren()`.

Firebase Storage (E6, image/video clue questions): wired up in code (`src/lib/firebase.ts`, `src/lib/storageMedia.ts`, `storage.rules`, `firebase.json`'s new `storage` block) but **not yet enabled for original POC** — `VITE_FIREBASE_STORAGE_BUCKET` is intentionally left blank in `.env.local`, so `getStorage()` never initializes and the host question editor shows "Media upload is not configured for this deployment" instead of an upload button. Enabling it for real requires: turning on Storage for the `legacy-firebase-project` Firebase project in the console (not yet done), filling in the bucket name, and running `npx firebase-tools deploy --only storage` to push `storage.rules`. Zero behavior change for original POC as shipped — see E6 under "Product Direction" for how this was verified against the real production game.

Hosting already deployed:

- Latest confirmed UI deploy to `the legacy Firebase Hosting site` is current as of `26395b5` (2026-07-18) — see "Current Git State" above for exactly what that includes. Re-verify against the commit list before assuming any change made *after* that commit is live.
...
- `src/lib/storageMedia.ts` - Firebase Storage upload helper for question media (`uploadQuestionMedia`, `isMediaUploadConfigured`); throws if Storage isn't configured for the deployment. Returns a plain `UploadedMediaFile` (`{type, url}`), not a full `QuestionMedia` — the upload step alone can't supply required alt text, see "Accessibility" under Known Follow-Ups. Deliberately has no delete/cleanup helper yet — see "Known Follow-Ups"
...
After the original POC event, the owner decided to turn this codebase into a resellable trivia-night product rather than a one-off original POC app: clone the repo per customer, reconfigure, redeploy under the customer's own Firebase project. Not a multi-tenant SaaS — no auth/tenant-isolation work is planned. A detailed implementation plan exists (was last saved to a local Claude Code plan file during a planning session; if that's not available, this section plus a fresh audit of any remaining customer-specific content is enough to reconstruct it). Two phases:

**Phase 1 — branding/copy/import/team-naming: DONE.**

- ✅ All customer-specific copy centralized into `src/config/site.ts` instead of scattered literal strings across `App.tsx`/`ScreenPage.tsx`/`PlayerPage.tsx`. Zero visible change for the current original POC event — verified live.
- ✅ `original POC-*` Tailwind tokens renamed to `brand-*` everywhere (`tailwind.config.js`, `src/styles.css`, 9 components). Verified live — dark/light theme, colors, all copy unchanged.
- ✅ Logo asset renamed to generic `src/assets/logo.png`.
- ✅ `template` branch created off `main` with customer-specific root files (`removed legacy logo asset`, question-bank docx/generator script) and internal docs (this file, `PROGRESS.md`, `IMPLEMENTATION_PLAN.md`, `legacy-firebase-project-codex-prompt.md`) removed. `main` keeps everything untouched as the live original POC instance. README updated with a "Setting up for a new customer" checklist. `template` is periodically merged forward from `main` to inherit generic (non-customer-specific) improvements — it is **not** automatically kept in sync, check its log.
...
- Hosting and database rules are confirmed current as of `26395b5` (see "Current Git State" / "Firebase Notes"); re-deploy again after any further source or rules changes.
...
- ✅ **Static a11y-scan pass done and fixed** (a full-repo markup review via the `/a11y-scan` skill, not live screen-reader/axe testing — see below for what's still open). Four findings, all fixed:
  - **Question media had no alt text anywhere** (the highest-severity finding — blocked screen-reader users from image-based questions entirely, a core flow). Root cause was structural: `QuestionMedia` (`src/types/domain.ts`) only ever had `{ type, url }`, so there was no field to even hold a description. Fixed by adding `altText: string` (**required**, non-empty, ≤250 chars) and `captionsUrl: string | null` (optional, for video) to `QuestionMedia`. `uploadQuestionMedia` (`src/lib/storageMedia.ts`) now returns a plain `UploadedMediaFile` (`{type, url}`, no `QuestionMedia`) since the upload step alone can't know a description — the host editor (`QuestionEditorRow` in `HostQuestionsPage.tsx`) holds `mediaFile`/`mediaAltText`/`mediaCaptionsUrl` as separate local state and combines them into the real `QuestionMedia` at save time; saving is blocked with a clear status message if media is attached but alt text is blank. Uploading a *new* file clears any previously-entered alt text/captions rather than carrying over a description that might no longer match — a deliberate choice to prevent a stale-description footgun. All 6 render sites that previously shipped `alt=""` (`PlayerPage.tsx` x3, `ScreenPage.tsx`'s shared `MediaClue`, `HostPage.tsx`, the editor's own thumbnail preview) now use `media.altText`. `database.rules.json`, `scripts/validateQuestions.mjs`, and bulk `.xlsx` import/export (`questionValidation.ts`/`questionImport.ts`, two new columns `mediaAltText`/`mediaCaptionsUrl`) all validate the new required field. Since Storage isn't enabled for original POC and none of the real 31 questions have media, this is zero-behavior-change for the live event — validated by `npm run validate:data` still passing untouched.
  - **Video clues had no caption mechanism.** Fixed via the same `captionsUrl` field — `ScreenPage.tsx`'s `MediaClue` renders `<track kind="captions" src={media.captionsUrl} default>` when present, plus `aria-label={media.altText}` on the `<video>` itself. Optional, not required (unlike alt text) — a host without a `.vtt` file ready can still attach a video, they just won't have captions yet.
  - **Single-select answer buttons used toggle (`aria-pressed`) semantics for what's actually a mutually-exclusive choice.** Fixed in `PlayerPage.tsx`'s `QuestionPanel`: the multiple-choice (not multi-select) answer grid is now a real `role="radiogroup"` of `role="radio"` buttons with `aria-checked`, roving `tabIndex` (only the selected radio, or the first one if none selected yet, is a tab stop), and arrow-key navigation (`ArrowUp`/`ArrowDown`/`ArrowLeft`/`ArrowRight` move both focus and selection, wrapping at the ends) — matching the real WAI-ARIA radiogroup interaction pattern rather than half-applying the role without the expected keyboard model. Multi-select questions were untouched — `aria-pressed` is the *correct* pattern there, since each choice toggles independently.
  - **Nested `<label>` elements** in the host editor's choice rows (`HostQuestionsPage.tsx`) — the outer `<label>` wrapped both the radio/checkbox input and a second, independently `for`/`id`-linked `<label>`+`<input>` pair for the choice text, which is invalid HTML and only worked by accident (the checkbox/radio's explicit `aria-label` happened to win in accessible-name computation). Fixed by wrapping only the checkbox/radio + its letter span in the inner `<label>`, with the choice-text `<label>`+`<input>` as siblings outside it — no nesting, same click-target UX.
  - Verified: full `validate:data`/`lint`/`build` pass; a temporary throwaway preview route exercised the real `QuestionEditorRow` (alt-text pre-fill, save-blocked-on-blank-alt, correct saved shape), the real `MediaClue` (confirmed the `<track>` element and its `src` actually render, confirmed image `alt` is correct), and the real `QuestionPanel` radiogroup (confirmed initial roving-tabindex distribution, confirmed arrow keys move both focus and `aria-checked` to the right button, confirmed tabindex re-rolls to only the newly-selected radio) — all without triggering any real Firebase writes (no Lock-In/Save actually invoked against production); then fully deleted. Real production `/screen`, `/host`, `/host/questions`, `/host/answer-key` re-checked against original POC's live game with zero console errors.
- Not yet done: a real screen-reader pass and automated tooling (axe/Lighthouse) against the live, running app — the a11y-scan above is static markup review, not a substitute for either.
...
The user previously had trouble with GitHub Desktop creating or looking for an extra nested project folder. The actual working repo root is:
...
The obsolete customer logo, customer-specific question-bank document and generator, and original scaffold prompt were removed from the working tree during the Trivia Knight cleanup. They remain recoverable from Git history if they are ever needed for archival reasons.
